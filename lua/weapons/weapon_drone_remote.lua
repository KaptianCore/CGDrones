...
SWEP.ViewModel = "models/ausdrone/c_controller/c_controller.mdl"
SWEP.WorldModel = "models/ausdrone/w_controller/w_controller.mdl"
SWEP.TargetDRR = NULL
SWEP.IndexDRR = 0
SWEP.WaitDRR = 0
...
function SWEP:Think()
    ...
    if CLIENT and self.Owner:KeyDown(IN_RELOAD) and CurTime() > self.WaitDRR and not self.Owner:GetNWEntity("DronesRewriteDrone"):IsValid() then
        ...
        if not IsValid(drone) then
            drone = self:GetNWEntity("DronesRewriteDrone")
        end
    ...
    timer.Create("request_drone" .. self:EntIndex(), self:SequenceDuration() * 0.35, 1, function()
        ...
        if tr.Hit and IsValid(tr.Entity) and tr.Entity.IS_DRR then
            ...
            local function foo()
                ...
                if #tab > 1 and IsValid(self.TargetDRR) and self.TargetDRR == old then
                    foo()
                end
            end

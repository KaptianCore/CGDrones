...
------------------------
function ENT:IsDroneOnGround()
    return self.isDroneOnGround
end
...
function ENT:GetUnit()
    return self.UNIT .. self:EntIndex()
end
...
function ENT:IsDroneDestroyed()
    return self:GetHealth() <= 0
end

function ENT:IsDroneImmortal()
    return self.Immortal or DRONES_REWRITE.ServerCVars.NoDamage:GetBool()
end

function ENT:IsDroneEnabled()
    return SERVER and self.Enabled or self:GetNWBool("Enabled")
end

function ENT:IsDroneWorkable()
    return self:HasFuel() and not self:IsDroneDestroyed() and self:IsDroneEnabled()
end

function ENT:GetHealth()
    return CLIENT and self:GetNWInt("Health") or self.HealthAmount
end

function ENT:GetDefaultHealth()
    return self:GetNWInt("DefHealth")
end

function ENT:GetFuel()
    return CLIENT and self:GetNWInt("Fuel") or self.Fuel
end

function ENT:HasFuel()
    return self.Fuel > 0 or not self.ShouldConsumeFuel or DRONES_REWRITE.ServerCVars.NoFuel:GetBool()
end
...
function ENT:GetDriver()
    return self:GetNWEntity("DronesRewriteDriver")
end

function ENT:GetCamera()
    return self:GetNWEntity("Camera")
end

function ENT:GetMainCamera()
    return self:GetNWEntity("MainCamera")
end

function ENT:DoCamEffects()
    return self:GetNWBool("CameraHUD") and not self:GetNWBool("ThirdPerson")
end

function ENT:SupportAngles()
    return self:GetNWBool("SupportAngles") and not self:GetNWBool("ThirdPerson")
end

function ENT:GetLocalCamPos()
    return self:GetNWVector("CameraPositions")
end

...
function ENT:DrawIndicator(index, val)
    if not self.HUD_indicators then
        self.HUD_indicators = {}
    end

    if not self.HUD_indicators[index] then
        self.HUD_indicators[index] = table.Count(self.HUD_indicators)
    end
...
function ENT:CallWeaponsMenu()
    if not LocalPlayer():IsAdmin() then
        LocalPlayer():ChatPrint("[Drones] You're not admin!")
...
function ENT:OpenWeaponsMenu(weps)
    ...
    for k, v in pairs(DRONES_REWRITE.Weapons) do
        ...
        -- k is 1 first not 0 but we need 0
        local btn = DRONES_REWRITE.CreateButton(k, 0, i * 31, 200, 30, wepslist, function()
            addWep = k
    ...
    local function AddWeps()
        ...
        for k, v in pairs(weps) do
            local btn = DRONES_REWRITE.CreateButton(v, 0, i * 31, 200, 30, existweps, function()
                ...
                menu:AddOption("Bind " .. addWep .. " weapon Attack1 to this " .. v .. " Attack1", function()
                    sync[v].fire1 = "fire1"
                end)

                menu:AddOption("Bind " .. addWep .. " weapon Attack1 to this " .. v .. " Attack2", function()
                    sync[v].fire2 = "fire1"
                end)

                menu:AddOption("Bind " .. addWep .. " weapon Attack2 to this " .. v .. " Attack1", function()
                    sync[v].fire1 = "fire2"
                end)

                menu:AddOption("Bind " .. addWep .. " weapon Attack2 to this " .. v .. " Attack2", function()
                    sync[v].fire2 = "fire2"
                end)

                menu:AddOption("Remove all chosen binds", function()
                    sync[v] = nil
                end)
    ...
    -- k is 1 first not 0 but we need 0
    local btn = DRONES_REWRITE.CreateButton("Add", 220, 60, 260, 40, win, function()
    ...
    local function UpdateBtns()
        ...
        for k, v in SortedPairsByMemberValue(self.Modules, "Slot", false) do
            ...
            remove.Paint = function(remove, w, h)
                ...
                if slot == "System" then
                    color = DRONES_REWRITE.Colors.DarkGrey
                end
...
function ENT:OpenSelectionMenu(weps)
    ...
    local function select()
        ...
        timer.Create("dronesrewrite_selectwep_gui_close" .. self:EntIndex(), 1.5, 1, function()
            close()
        end)

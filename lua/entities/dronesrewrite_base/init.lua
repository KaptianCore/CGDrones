...
function ENT:IsEnemy(class)
    ...
    if DRONES_REWRITE.ServerCVars.NPCReverse:GetBool() then
        enemycheck = IsFriendEntityName(class)
    end
...
function ENT:Key(key)
    ...
    if self.KeysFuncs.Physics[key] then
        if self:CanUsePhysicsKeys() then
            self.KeysFuncs.Physics[key](self)
        end
    ...
    for k, v in pairs(self.Keys) do
        if self.KeysFuncs.Physics[k] then
            ...
            elseif v._Pressed then
                ...
                if self.KeysFuncs.UnPressed[k] then
                    self.KeysFuncs.UnPressed[k](self)
                end
    ...
    if ply:IsValid() then
        ...
        if self:CanBeControlledBy(ply) then
            ...
            if weapon:IsValid() then
                self.PlyOldWeapon = weapon:GetClass()
            end

            if not ply:HasWeapon("weapon_drone_remote") then
                ply:Give("weapon_drone_remote")
            end

            ply:SelectWeapon("weapon_drone_remote")
            ...
            if curWep:IsValid() then
                ...
                if curWep.Tab.Deploy then
                    curWep.Tab.Deploy(self, curWep)
                end
            end
...
function ENT:AddModule(name)
    ...
    if not module.System then
        self:SetNWBool("hasModule" .. name, true)
    end

    if module.Slot then
        self:SetNWInt("SlotCount" .. module.Slot, self:CountModulesInSlot(module.Slot))
    end
...
function ENT:RemoveModule(name)
    ...
    if not module.System then
        self:SetNWBool("hasModule" .. name, false)
    end
...
function ENT:RemoveWeapon(name)
    ...
    if wep and wep:IsValid() then
        ...
        if wep.Tab.OnRemove then
            wep.Tab.OnRemove(self, wep)
        end
    end
...
function ENT:SelectNextWeapon(name)
    ...
    if IsValid(oldWep) and oldWep.Tab.Holster then
        oldWep.Tab.Holster(self, oldWep)
    end
        ...
        if not self:IsDroneDestroyed() and self.EnginePower > 0 and self:HasPropellers() then
            ...
            for k, v in pairs(self.ValidPropellers) do
                if v:IsValid() and not v.IsDestroyed then
                    ...
                    angForce = angForce - Vector(angr, 0, 0) * 0.1 * self.AlignmentRoll - Vector(0, angp, 0) * 0.1 * self.AlignmentPitch - Vector(angr, angp, 0) * 0.1 * self.Alignment
            ...
            if not DRONES_REWRITE.ServerCVars.NoNoise:GetBool() then
                ...
                if not self.RandomNoiseValue then
                    self.RandomNoiseValue = math.random(-1, 1)
                end

                -- ???
                if self.RandomNoiseValue == 0 then
                    self.RandomNoiseValue = 1
                end
            ...
            angForce = angForce - angVel * 0.05 * self.AngDamping - Vector(angVel.x * self.AngRollDamping, angVel.y * self.AngPitchDamping, angVel.z * self.AngYawDamping) * 0.05
            self:ReduceFuel()
        end
    elseif self.DrrBaseType == "walker" then
...
function ENT:Think()
    if not self.DRONES_REWRITE_DELTA then
        self.DRONES_REWRITE_DELTA = 0
    end
    ...
    if self:IsDroneWorkable() and not DRONES_REWRITE.ServerCVars.NoHitPropellers:GetBool() and not self.NoPropellers and self:GetCollisionGroup() ~= COLLISION_GROUP_WORLD then
        if not self.PropellerHitWait then
            self.PropellerHitWait = 0
        end
    ...
    if driver:IsValid() then
        ...
        if vehicle:IsValid() then
            eang = vehicle:WorldToLocalAngles(driver:EyeAngles())
        end
        ...
        if not DRONES_REWRITE.ServerCVars.NoCameraRestrictions:GetBool() then
            if DRONES_REWRITE.ServerCVars.StaticCam:GetBool() then
                ...
                if self.AllowPitchRestrictions then
                    if eang.p < ang.p + self.PitchMin then
                        eang.p = ang.p + self.PitchMin
                    end

                    if eang.p > ang.p + self.PitchMax then
                        eang.p = ang.p + self.PitchMax
                    end
                end

                if self.AllowYawRestrictions then
                    if eang.y < ang.y + self.YawMin then
                        eang.y = ang.y + self.YawMin
                    end

                    if eang.y > ang.y + self.YawMax then
                        eang.y = ang.y + self.YawMax
                    end
                end
            else
                if self.AllowPitchRestrictions then
                    if eang.p < self.PitchMin then
                        eang.p = self.PitchMin
                    end

                    if eang.p > self.PitchMax then
                        eang.p = self.PitchMax
                    end
                end

                if self.AllowYawRestrictions then
                    if eang.y < self.YawMin then
                        eang.y = self.YawMin
                    end

                    if eang.y > self.YawMax then
                        eang.y = self.YawMax
                    end
                end
            end
    ...
    if self.Camera:IsValid() and self:IsDroneWorkable() then
        self.Camera:SetAngles(self.CamAngles)
    end
    ...
    if self:IsDroneWorkable() then
        if self.StopKeysFix then
            self.StopKeysFix = false
        end
    else
